...
class MapboxService {
    ...
    /// Note: Address validation is done before calling this method
    /// If geocoding fails, returns a mock ETA instead of throwing an error
    func calculateETA(for address: DeliveryAddress) async throws -> Int {
        ...
        // Try to geocode and calculate ETA, but fall back to mock if geocoding fails
        do {
            // Geocode the delivery address (no validation, just geocoding for ETA calculation)
            ...
            // If geocoding fails, return mock ETA instead of throwing error
            // This allows orders to proceed even if Mapbox API has issues
            try await Task.sleep(nanoseconds: 500_000_000) // 0.5 seconds
            return Int.random(in: 20...40)
        }
    }
    ...
    /// Returns coordinates if successful, throws network error if geocoding fails
    private func geocodeAddress(_ address: DeliveryAddress) async throws -> (Double, Double) {
        ...
        guard let url = URL(string: urlString) else {
            throw AppError.networkError("Invalid geocoding URL")
        }
        ...
        do {
            ...
            guard let feature = response.features.first,
                ...
                throw AppError.networkError("No geocoding results found")
            }
            ...
            throw AppError.networkError("Geocoding failed: \(error.localizedDescription)")
        }
    }
